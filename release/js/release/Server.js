...
var com_tamina_cow4_Server = function() {
	...
	while(_g1 < _g) {
		...
		case "-p1_pos":
			...
			com_tamina_cow4_config_Config.MODE_DEBUG = true;
			break;
		case "-p2_pos":
			posArg = this._process.argv[i + 1].split(",");
	...
	haxe_Log.trace("server listening on " + 3000,{ fileName : "Server.hx", lineNumber : 74, className : "com.tamina.cow4.Server", methodName : "new"});
...
var com_tamina_cow4_socket_SheepIA = function() {
	...
	this.items = [];
	this.turnComplete = new msignal_Signal1();
	this._isFirstTurn = !com_tamina_cow4_config_Config.MODE_DEBUG;
};
...
com_tamina_cow4_socket_SheepIA.prototype = {
	...
	,getTurnOrder: function(data) {
		...
		try {
			...
			if(this._isFirstTurn) this.initFirstTurn();
			if(this._currentPath == null || this._currentPath.getLastItem().id == currentCell.id) this._currentPath = this.getNewDestination();
	...
	,initFirstTurn: function() {
		...
		this._currentPath = com_tamina_cow4_utils_GameUtils.getPath(currentCell,this._data.getCellAt(0,12),this._data);
		this._isFirstTurn = false;
	}
	,getNextIntersection: function(fromCell,byCell,path) {
		path.push(fromCell);
		if(byCell != null) {
			var neighbors = byCell.getNeighboors();
			if(neighbors.length == 1 || neighbors.length > 2) path.push(byCell); else {
	...
	,getNewDestination: function() {
		var result = new com_tamina_cow4_model_Path();
		var currentCell = this._data.getCellByIA(this.id);
		...
		this.getNextIntersection(currentCell,selectedNeighbor,result);
		return result;
	}
	,getItemByType: function(type) {
		var result = null;
