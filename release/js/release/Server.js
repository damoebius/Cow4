...
var com_tamina_cow4_Server = function() {
	com_tamina_cow4_config_Config.ROOT_PATH = nodejs_NodeJS.get_dirname() + "/../../server/";
	...
	while(_g1 < _g) {
		...
		switch(arg) {
		case "-p1_pos":
			posArg = this._process.argv[i + 1].split(",");
			com_tamina_cow4_config_Config.PLAYER_1_START_POSITION = new com_tamina_cow4_model_vo_Position(posArg[0],posArg[1]);
			break;
		case "-p2_pos":
			posArg = this._process.argv[i + 1].split(",");
			com_tamina_cow4_config_Config.PLAYER_2_START_POSITION = new com_tamina_cow4_model_vo_Position(posArg[0],posArg[1]);
			break;
		case "-c_pos":
			posArg = this._process.argv[i + 1].split(",");
			com_tamina_cow4_config_Config.SHEEP_START_POSITION = new com_tamina_cow4_model_vo_Position(posArg[0],posArg[1]);
			break;
		}
	}
	this._express = nodejs_express_Express.GetApplication();
	this._express.listen(3000);
	this._express["use"](nodejs_express_Express.Static(com_tamina_cow4_config_Config.ROOT_PATH));
	...
	haxe_Log.trace("server listening on " + 3000,{ fileName : "Server.hx", lineNumber : 71, className : "com.tamina.cow4.Server", methodName : "new"});
...
var com_tamina_cow4_core_Game = function(iaList,gameId,player) {
	...
	this._data.getCellAt(com_tamina_cow4_config_Config.PLAYER_1_START_POSITION.x,com_tamina_cow4_config_Config.PLAYER_1_START_POSITION.y).set_occupant(this._IAList[0].toInfo());
	this._data.getCellAt(com_tamina_cow4_config_Config.PLAYER_2_START_POSITION.x,com_tamina_cow4_config_Config.PLAYER_2_START_POSITION.y).set_occupant(this._IAList[1].toInfo());
	this._data.getCellAt(com_tamina_cow4_config_Config.SHEEP_START_POSITION.x,com_tamina_cow4_config_Config.SHEEP_START_POSITION.y).set_occupant(this._sheep.toInfo());
};
	...
	,retrieveIAOrders: function(targetIA) {
		...
		while(_g1 < _g) {
			...
			while(_g3 < _g2) {
				var j = _g3++;
				var cell = columns[j];
				if(cell.get_occupant() != null && cell.get_occupant().invisibilityDuration > 0) haxe_Log.trace("INVISIBLE !!!",{ fileName : "Game.hx", lineNumber : 115, className : "com.tamina.cow4.core.Game", methodName : "retrieveIAOrders"});
			}
		}
		...
		while(_g11 < _g4) {
			...
			while(_g31 < _g21) {
				...
				if(cell1.get_occupant() != null) haxe_Log.trace("cell occupée",{ fileName : "Game.hx", lineNumber : 126, className : "com.tamina.cow4.core.Game", methodName : "retrieveIAOrders"});
				if(cell1.get_occupant() != null && cell1.get_occupant().invisibilityDuration > 0) haxe_Log.trace("INVISIBLE !!!",{ fileName : "Game.hx", lineNumber : 129, className : "com.tamina.cow4.core.Game", methodName : "retrieveIAOrders"});
...
com_tamina_cow4_routes_MainRoute.prototype = $extend(com_tamina_cow4_routes_Route.prototype,{
	_sucessHandler: function(request,response) {
		var path = nodejs_NodeJS.require("path");
		response.sendfile(path.resolve(com_tamina_cow4_config_Config.ROOT_PATH + "index.html"));
	}
...
com_tamina_cow4_routes_PlayRoute.prototype = $extend(com_tamina_cow4_routes_Route.prototype,{
	_sucessHandler: function(request,response) {
		if(request.param("ia1") != null && request.param("ia2") != null && request.param("gameId") != null) {
			var path = nodejs_NodeJS.require("path");
			response.sendfile(path.resolve(com_tamina_cow4_config_Config.ROOT_PATH + "play.html"));
		} else console.error("ERROR : url params not found");
	}
...
com_tamina_cow4_model_GameConstants.INVISIBILITY_DURATION = 10;
